...
void game_loop(void) {
    ...
    /* Screen memory still holds whatever SuperBASIC's mode 4 console left
     * there before takeover; mode 8 reinterprets those bytes as pixels
     * (including stray flash bits), which is the striping/flicker seen
     * without this clear. */
    fill_screen(0, 0);
    while (!chosen) {
